            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3>Tambah Trip Penyaluran Donasi</h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('trip_penyaluran_donasi.store') }}" method="POST">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <select name="pondok_id" class="form-control" required>
                                    ...
                                    @foreach($pondoks as $pondok)
                                        <option value="{{ $pondok->id }}">{{ $pondok->hasilSurvey->calonMitra->nama_pondok}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="donasi_masuk_id" class="form-control" required>
                                    ...
                                    @foreach($donasiMasuks as $donasiMasuk)
                                        <option value="{{ $donasiMasuk->id }}">{{ $donasiMasuk->donasi_yang_masuk }}</option>
                                    @endforeach
                                </select>
                            </div>
